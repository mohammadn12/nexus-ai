...
def seed_ultimate_free_tools():
    try:
        ...
        # Create basic schema safely if dropped
        cursor.execute('CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY, name TEXT NOT NULL, slug TEXT NOT NULL UNIQUE)')
        cursor.execute('CREATE TABLE IF NOT EXISTS tools (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, tagline TEXT NOT NULL, website_url TEXT NOT NULL, category_id INTEGER, pricing TEXT, featured TEXT, upvotes INTEGER DEFAULT 0, FOREIGN KEY (category_id) REFERENCES categories (id))')
        ...
        # Minimum hardcoded list to avoid any mobile copy-paste truncation errors
        tools = [
...
def api_get_tools():
    ...
    tools = database.get_tools(query=q if q else None, category_slug=category if category != 'all' else None, pricing=pricing if pricing != 'all' else None, featured=featured, sort_by=sort_by)
...
def api_get_tool(tool_id):
    ...
    if not tool: return jsonify({'status': 'error', 'message': 'Tool not found'}), 404
...
def api_upvote_tool(tool_id):
    ...
    if new_count is None: return jsonify({'status': 'error', 'message': 'Tool not found'}), 404
    return jsonify({'status': 'success', 'upvotes': new_count})

@app.route('/api/tools/submit', methods=['POST'])
def api_submit_tool():
    data = request.get_json()
    if not data: return jsonify({'status': 'error', 'message': 'Invalid JSON body'}), 400
    required_fields = ['name', 'tagline', 'website_url', 'category_id']
    for field in required_fields:
        if not data.get(field): return jsonify({'status': 'error', 'message': f'Field "{field}" is required'}), 420
    try:
...
def admin_login():
    ...
    return '<div style="max-width:300px; margin:100px auto; padding:20px; border:1px solid #ccc; text-align:center; font-family:sans-serif;"><h2>Admin Login</h2><form method="POST"><input type="text" name="username" placeholder="Username" required style="width:100%; margin-bottom:10px; padding:8px;"><br><input type="password" name="password" placeholder="Password" required style="width:100%; margin-bottom:10px; padding:8px;"><br><button type="submit" style="width:100%; padding:10px; background:#ff007f; color:#fff; border:none; cursor:pointer;">Login</button></form></div>'

@app.route('/secret-admin/dashboard')
def admin_dashboard():
    if not session.get('admin_logged_in'): return redirect(url_for('admin_login'))
    tools = database.get_tools()
    html = '<div style="padding:20px; font-family:sans-serif; max-width:800px; margin:0 auto;"><h2>NexusAI Admin Dashboard</h2><p><a href="/secret-admin/logout" style="color:red;">Logout</a></p><table border="1" cellpadding="10" style="width:100%; border-collapse:collapse;"><tr><th>Tool Name</th><th>Category</th><th>Action</th></tr>'
    for tool in tools:
...
def delete_tool(id):
    ...
    try:
        ...
        conn.commit()
        conn.close()
    except Exception as e: return f"Error deleting tool: {str(e)}"
    return redirect(url_for('admin_dashboard'))
